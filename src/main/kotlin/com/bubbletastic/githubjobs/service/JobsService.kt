...
class JobsService(private val networkHandler: NetworkHandler = NetworkHandlerKhttp()) {
    ...
    internal fun accumulatePages(queryUrl: String): JsonArray<JsonObject> {
        var morePages = true
        var pageNum = 0
        val pages = JsonArray<JsonObject>()
        while (morePages) {
            val input = networkHandler.get("$queryUrl&page=$pageNum")
            val page = Parser().parse(input) as JsonArray<JsonObject>
            pages.addAll(page)
